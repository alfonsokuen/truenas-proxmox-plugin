...
#
# 06-status-budget.t fixed the retry storm inside status(): both lookups now
# run with retry_max => 0. That still leaves one probe per poll per storage,
# and one probe against a dead API costs a full broker deadline. pvestatd is
# a single process that walks every storage every 10 seconds, so with two
# truenasplugin storages configured that is the whole metrics pipeline
# stopped for the length of the outage. Measured with tools/i32-apiloss.sh:
# pvestatd wrote no metric at all while one array was unreachable, and four
# other storages stopped answering with it.
#
# The fix has two halves and this file drives both:
#   - a short budget for the probe, because nobody is waiting on a capacity
#     figure and a stale metric costs less than a blocked pvestatd;
#   - a marker: after a connectivity failure, answer inactive from memory for
#     tn_status_probe_backoff_s without touching the network.
#
...
#
# Assertions tagged EXPECT-FAIL(unfixed) must FAIL without the marker.
#
# Run with:  prove -v t/nvme/13-status-probe-backoff.t
...

{
    ...
    # EXPECT-FAIL(unfixed): without the marker this probes again every time.
