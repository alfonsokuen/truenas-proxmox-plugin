...
#
# 13-status-probe-backoff.t proved the marker stops status() from re-probing a
# dead array. It does not cover the caller that actually dominates the cost:
# PVE::Storage::storage_info() calls activate_storage() BEFORE status() on every
# sweep, and the nvme-tcp branch calls _nvme_ensure_subsystem(), whose first act
# is an unconditional nvmet.subsys.query. Measured on pve3 2026-08-23 with the
# marker working perfectly: 15 of 15 pvestatd polls paid ~10s each there, while
# status() returned instantly. The marker was suppressing 73% of its own probes
# and none of the real bill.
#
# The first attempt at this SKIPPED the ensure while the marker stood, and was
# rejected in review: _nvme_ensure_subsystem() is the only periodic caller of
# _nvme_sync_portals(), which republishes ports the target lost - and that cache
# is invalidated exactly when a connect fails, i.e. precisely when the skip would
# kick in. It also stranded _nvme_ensure_host_registered() in whitelist mode, the
# one path that registers this node's NQN. So the contract under test is:
#
...
#
# Run with:  prove -v t/nvme/14-activate-storage-marker.t
